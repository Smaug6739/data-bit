...
class DataBit {
    ...
    /**
     * Check if the data manager have flags.
     * @returns {boolean} true if the data manager has flags
     */
    hasAnyflags() {
        if (this.bits > 0)
            return true;
    ...
    removeFlag(flagToRemove) {
        ...
        if (Array.isArray(flagToRemove))
            return flagToRemove.every(pta => this.removeFlag(pta));
        if (type === 'string') {
